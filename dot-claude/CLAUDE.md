...
- Only start implementing straight off if the implementation requires trivial changes, otherwise always go into plan mode, explore the codebase, write up a plan and then start working on it.
...
- Explay **why** something was done in a short commit message body (1-2 paragraphs), never _what_ was done (it's in the diff).
